...
<body>
   ...
   <section class="content" id="bodyContainer">
      ...
      <table>
         <tr><th colspan="9">Orders List</th></tr>
         <tr>
            <th>ID</th>
            <th>ID_Clt</th>
            <th>ID_Prd</th>
         ...
         <?php
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
               if ($row['delivered']=='yes'){
                  $classColor="oui";
                  $valueButton = "Delivered on : ".$row['date_delivery'];
                  $controller = "";

               }else{
                  $classColor="non";
                  $valueButton = "Mark as Delivered";
                  $controller = "changeToDelivered.php";
               }
         ?>
         <tr>
            <td class="<?php echo $classColor ?>"><?php echo $row['id_commande'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['id_client'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['id_product'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['qte']?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['id_payment'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['id_payer'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo $row['date_commande'] ?></td>
            <td class="<?php echo $classColor ?>"><?php echo strtoupper($row['delivered']) ?></td>
            <td class="<?php echo $classColor ?>"><form style="margin: 0;" action="<?php echo $controller?>" method="POST"><button class="display" type="submit" name='id_to_change' value="<?php echo $row['id_commande'] ?>"><?php echo $valueButton ?></button></form></td>
         </tr>
         <?php } ?>
      </table>
