...
if($_COOKIE['id_payment'] && $_COOKIE['status_payment'] && $_COOKIE['value_payment'] && $_COOKIE['currency_payment'] && $_COOKIE['id_client'] && $_COOKIE['payer_id'] && $_COOKIE['payer_address'] && $_COOKIE['payer_name']){
   ...
   $allDone = 0;
   // return cart's elements
   $sql = "SELECT * FROM cart WHERE id_client = $id_client";
   ...
   while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
      $id_product = $row['id_produit'];
      $qty = $row['qty'];
      $sql5 = "SELECT * FROM produit WHERE id_produit = $id_product";
      $stmt5 = $conn->prepare($sql5);
      $stmt5->execute();
      $row5 = $stmt5->fetch(PDO::FETCH_ASSOC);
      array_push($products_array, $row5['nom_produit']);
      array_push($prices_array, $row5['nouveau_prix']);
      array_push($quantities_array, $row['qty']);
      array_push($totals_array, $row['total']);

      $valuPayment = $row['qty']*$row5['nouveau_prix'];

      // enregistrer le paiement
      $sql1 = "INSERT INTO payments(id_payment , status_payment , value_payment , currency_payment , id_product , qty) VALUES('$id_payment','$status_payment',$valuPayment,'$currency_payment',$id_product,$qty)";
      $stmt1 = $conn->prepare($sql1);

      // enregistrer la commande
      $sql2 = "INSERT INTO commande(id_product , id_client , qte , address_payer , id_payment , id_payer) VALUES($id_product,$id_client,$qty,'$payer_address','$id_payment','$payer_id')";
   ...
   $total = 0;
   if($allDone == count($products_array)){
      
      foreach ($totals_array as $valeur) {
         $total = $total + $valeur;
      }

      $sql5="DELETE FROM cart WHERE id_client = $id_client";
      $stmt5 = $conn->prepare($sql5);
      if($stmt5->execute()){

         // generate pdf
         $date = date('d-m-y h:i:s');
         $nbr_products = count($products_array);

         $sql6 = "SELECT * FROM client WHERE id_client = $id_client";
         ...
         <body>
            ...
            <div class="content">
               ...
               <table>
                  ...
                  <tbody>';
                     for($i = 0 ; $i<$nbr_products ; $i++){
                        $html .= '<tr>
                        <td>'.$products_array[$i].'</td>
                        <td>'.$quantities_array[$i].'</td>
                        <td>'.number_format($prices_array[$i] , 2).'</td>
                        <td>'.number_format($totals_array[$i] , 2).'</td>
                        </tr>';
                     }
...
<head>
   ...
   <style>
      ...
      .content table tr:first-child th , .content table #totalAPayer{
         background-color: #cd313b;
         color: white;
      }
...
<body>
   ...
   <section class="content">
      <table>
         ...
         ?>
         <tr>
            <td><?php echo $products_array[$i]; ?></td>
            <td><?php echo $quantities_array[$i]; ?></td>
            <td><?php echo $prices_array[$i]; ?></td>
            <td><?php echo $totals_array[$i]; ?></td>
         </tr>
         <?php } ?>
         <tr></tr>
         <tr><td colspan="2"></td><th id="totalAPayer">Total</th><th id="totalAPayer"><?php echo $total."$" ?></th></tr>
         <tr></tr>
         <tr><td colspan="4">YOUR DEVICE - THANK YOU FOR YOUR CONFIDENCE</td></tr>
      </table>
...
<?php 
            ...
            header("Location:index.php?errorpaymentErrorDelete");
            exit();
         }
         
      }else{
         header("Location:index.php?errorpaymentNotAllDone");
         exit();
      }

   }else{
      header("Location:index.php?errorpaymentNotco");
      exit();
   }
}else{
   header("Location:clientLogin?errorpaymentNotLogin");
   exit();
}
?>